...
std::vector<uint8_t> recover(uint8_t v_new, std::vector<uint8_t> r, std::vector<uint8_t> s, std::vector<uint8_t> _message) {
    ...
    if (!secp256k1_ecdsa_recoverable_signature_parse_compact(ctx, &rawSig, reinterpret_cast<uint8_t *>(&signature[0]), v)) {
        printf("No secp256k1_ecdsa_recoverable_signature\n");
        return {};
    }

    secp256k1_pubkey rawPubkey;
    if (!secp256k1_ecdsa_recover(ctx, &rawPubkey, &rawSig, reinterpret_cast<uint8_t *>(&_message[0]))) {
